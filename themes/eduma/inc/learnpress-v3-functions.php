	...
	function thim_remove_learnpress_hooks() {
        ...
        add_action('init', function (){
            if ( ( thim_plugin_active( 'learnpress-wishlist/learnpress-wishlist.php' && class_exists( 'LP_Addon_Wishlist' ) ) && is_user_logged_in() ) && thim_is_version_addons_wishlist( '3' ) ) {
                ...
                remove_action('learn-press/content-learning-summary', array($instance_addon, 'wishlist_button'), 100);
            }
        }, 99);
...
if(!function_exists('thim_content_item_lesson_complete_button')) {
    function thim_content_item_lesson_complete_button() {
        $user   = LP_Global::user();
        $course = LP_Global::course();
        ...
        if ( $lesson_item = $course_item->get_item( $lesson->get_id() ) ) {
            if ( $lesson_item->is_completed() ) {
                ?>
                <div class="message  message-success"><?php _e( 'You have completed this lesson.', 'eduma' );?></div>
                <?php
                return;
            }
        }
...
add_action( 'learn-press/after-content-item-summary/lp_lesson', 'thim_content_item_lesson_complete_button', 12 );
...
if( !function_exists( 'thim_checkout_link_login_register' ) ) {
    function thim_checkout_link_login_register() {
        ...
        echo 'You can <a href="' . esc_attr( $link_login ) . '">' . __( 'login', 'eduma' ) . '</a> now. Don\'t have an account? Click <a href="' . esc_attr( $link_register ) . '">' . __( 'register now', 'eduma' ) . '</a>';
        echo '</div>';
    }
}
